...
function New-GraficaLineas {
    ...
    $area.AxisX.LabelStyle.Angle = -45
    $area.AxisX.MajorGrid.Enabled = $false
    # En una tendencia si ayuda la cuadricula horizontal para comparar niveles
    # entre fechas -no compite con etiquetas de valor como en las barras-.
...
# -si no, las lineas quedarian desalineadas entre si-.
...
try {
    ...
    # Los defaults evitan que un config_correo_backlog.json viejo (sin estas
    # llaves nuevas) rompa el script.
    ...
    # Con un solo punto no hay "tendencia" que mostrar -una linea necesita al
    # menos dos cortes-; en ese caso se pone un aviso en vez de la grafica.
...
$kpis
<h3 style='color:#1f4e78'>Tendencia del backlog total</h3>
$bloqueTendencia
